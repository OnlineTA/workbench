...
sufficient for a pleasant assessment code development environment.
...
enable a development environment where the teachers could dry-run their
assessment code, not just against a mimic testing interface, but an exact
replica of the OnlineTA testing and sandboxing infrastructure. This would
